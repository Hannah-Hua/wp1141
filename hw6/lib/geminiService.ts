...
// 初始化 Gemini API
// 注意：如果使用 Google AI Studio 的 API Key，可能需要使用不同的端點
...
export async function generateGameTurn(
  ...
  try {
    const model = genAI.getGenerativeModel({ 
      model: 'gemini-2.5-flash', // 使用免費的 gemini-2.5-flash 模型
    });
    
    const systemPrompt = getSystemPrompt(gameState);
    
    // 建立完整的提示詞（Gemini 需要將系統提示詞包含在使用者訊息中）
    ...
    // 解析 JSON 回應
    let gameResult: GameTurnResult;
    try {
      gameResult = JSON.parse(cleanedText) as GameTurnResult;
    } catch (parseError) {
      ...
      if (jsonMatch) {
        try {
          gameResult = JSON.parse(jsonMatch[0]) as GameTurnResult;
          console.warn('Gemini response contained extra text, but JSON 部分已解析成功');
        } catch (innerError) {
          console.error('Second JSON parse attempt failed:', innerError);
          throw new Error('GEMINI_JSON_PARSE_ERROR');
        }
      } else {
        throw new Error('GEMINI_JSON_PARSE_ERROR');
      }
    ...
    if (!gameResult.narration || !Array.isArray(gameResult.options) || !gameResult.effects) {
      console.error('Gemini Invalid Format:', {
        hasNarration: !!gameResult.narration,
        hasOptions: Array.isArray(gameResult.options),
        hasEffects: !!gameResult.effects,
        result: gameResult,
      });
    ...
    // 確保至少有 2 個選項
    if (gameResult.options.length < 2) {
      console.warn('Gemini response has less than 2 options, adding default options', {
        originalOptions: gameResult.options,
      });
      // 如果選項太少，補充預設選項
      gameResult.options = [
        ...gameResult.options,
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('503') || rawStatus === 503 || error.message.includes('overloaded')) {
        throw new Error('GEMINI_SERVICE_UNAVAILABLE');
      }
      // 保留原始錯誤訊息以便除錯
      if (error.message === 'GEMINI_JSON_PARSE_ERROR' || error.message === 'GEMINI_INVALID_FORMAT') {
