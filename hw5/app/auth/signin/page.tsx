...
export default function SignIn() {
  ...
  const handleUserLogin = async (userId: string) => {
    try {
      ...
      if (!provider) {
        setError('該用戶的 Provider 資訊不存在');
        return;
      }

      // 跳轉到對應的 Provider 登入頁面
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-lg shadow-lg">
        ...
        {/* 註冊區塊 */}
        <div>
          <p className="text-sm text-gray-600 mb-4 text-center">還沒有帳號？選擇 Provider 進行註冊</p>
          <div className="space-y-3">
            <button
              onClick={() => signIn('google', { callbackUrl: '/auth/register' })}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 rounded-full shadow-sm bg-white hover:bg-gray-50 transition-colors"
            >
